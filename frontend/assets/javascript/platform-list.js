...
function appendPlatformCard(platform) {
    ...
    var id = platform._id;
    var url = platform.url;
    text = `<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 platform-card" id="${id}-name">`;
    text += `<div class="admin-content analysis-progrebar-ctn res-mg-t-15">`;
    text += `<h2 class="text-left text-uppercase"><b>${name}<b></h2>`;
    text += `<div class="row vertical-center-box vertical-center-box-tablet">`;
    text += `<div class="col-xs-9 cus-gh-hd-pro">`;
    text += `<h4 class="text-left no-margin">${url}</h4>`;
...
function appendPlatformCard(platform) {
    ...
    var id = platform._id;
    var url = platform.url;
    text = `<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 platform-card" id="platform-card">`;
    text += `<div class="admin-content analysis-progrebar-ctn res-mg-t-15" id="${id}-name">`;
    text += `<h3 class="text-left text-wrap text-uppercase"><b>${name}<b><i class="fas fa-cubes" style="margin-left: 10px;"></i></h3>`;
    text += `<div class="row vertical-center-box vertical-center-box-tablet">`;
    text += `<div class="col-xs-9 cus-gh-hd-pro">`;
    text += `<p class="text-left"><i class="fas fa-link" style="margin-right: 10px;"></i> ${url}</p>`;
    text += `</div></div></div></div>`;


    // $(`#${id}-name`).click(function() {
    //     // window.location = '/platform.html?url=' + platform.url + '&name=' + platform.name;
    //     console.log("click");
    // });
